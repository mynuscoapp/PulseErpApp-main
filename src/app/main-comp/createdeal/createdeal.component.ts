...
export class CreatedealComponent {
  ...
  createColumnsDefinition() {
    this.colDefs = [
      {
        headerName: 'Product', field: 'productName', sortable: true, width: 280, resizable: true, filter: true, editable: true, cellEditor: 'agRichSelectCellEditor', cellEditorParams: {
          values: this.productNamesList,
          searchType: "matchAny",
          allowTyping: true,
          filterList: true,
          highlightMatch: true,
  ...
  private GetProductRowsForUpdate(data: DealProductList[]) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (isNaN(this.productTotalPrice)) {
        ...
        //productRow.total = this.productTotalPrice.toFixed(2);
      }
